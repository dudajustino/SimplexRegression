...
options(width = 70, prompt = "R> ", continue = "+  ", digits = 5)
```
...
(5) normal Q-Q plot; (6) worm plot; (7) Cook's distances; (8) generalized
leverages. The default residual type is `"quantile"`. For plots 7 and 8,
`threshold` highlights influential observations and `label.pos` controls
label placement (1 = below, 2 = left, 3 = above, 4 = right).

```{r plots-1-6, fig.height = 8, fig.cap = "Diagnostic plots (1–6) for the fitted simplex regression model with log-log link."}
par(mfrow = c(3, 2))
plot(fit_loglog, which = 1:6, reset.par = FALSE)
```

```{r plot-cook, fig.height = 4, fig.cap = "Cook's distances. Observations exceeding the threshold of 0.15 are labeled."}
plot(fit_loglog, which = 7, threshold = 0.15, label.pos = 4)
```

```{r plot-glev, fig.height = 4, fig.cap = "Generalized leverage values. Observations exceeding 0.08 are labeled."}
plot(fit_loglog, which = 8, threshold = 0.08, label.pos = 3)
```
...
## Information matrix-based measures (`diag.im`)

The `diag.im` function computes leave-one-out influence measures $s_{3,i}$
and $s_{5,i}$, proposed by Cribari-Neto, Vasconcellos and Santana e Silva
(2025). For each observation $i$, the model is refit without observation $i$
and the deviation from the full-data estimates is quantified through
information matrix comparisons. The `type` argument selects which
measure(s) to compute (`"s3"`, `"s5"`, or both). Detection thresholds are
controlled by `interval`: `"I1"` for moderate detection and `"I2"` for
stricter detection. The `verbose` argument controls whether progress
messages are printed during the leave-one-out refitting.

```{r diag-im-s3, fig.height = 5, fig.cap = "Index plot of the $s_{3,i}$ measure with detection threshold I2."}
diag.im(
  fit_loglog,
  data      = rh,
  type      = "s3",
  ...
  verbose   = FALSE,
  plot      = TRUE,
  label.pos = c(1, 4, 3, 3, 3, 2, 3)
)
```

```{r diag-im-s5, fig.height = 5, fig.cap = "Index plot of the $s_{5,i}$ measure with detection threshold I2."}
diag.im(
  fit_loglog,
  data      = rh,
  type      = "s5",
  ...
  verbose   = FALSE,
  plot      = TRUE,
  label.pos = c(3, 1, 1, 2, 1, 1, 1)
)
```

## Distance-based measures (`diag.distances`)

The `diag.distances` function computes leave-one-out influence diagnostics
based on distributional distances between the full-data and leave-one-out
fitted simplex distributions. Three distances are available via `type`:
Hellinger (`"H"`), Wasserstein with $p_W = 1$ (`"W1"`), and Wasserstein
with $p_W = 2$ (`"W2"`). Influential observations are flagged using an
ad hoc threshold based on an asymmetric IQR rule. The `verbose` argument
controls progress reporting.

```{r diag-dist-H, fig.height = 5, fig.cap = "Hellinger distance between full-data and leave-one-out fitted simplex distributions."}
diag.distances(
  fit_loglog,
  data      = rh,
  type      = "H",
...
halfnormal.plot(fit_loglog, nsim = 100, type = "weighted", seed = 2026)
```

...
The impunity dataset contains information about various socioeconomic and political indicators across countries, with the response variable measuring levels of impunity (a bounded variable in the open interval (0,1)).
...
One of the key features of simplex regression is the ability to use parametric mean link functions that adapt to the data structure. These functions include an additional parameter (lambda) that is estimated from the data, providing greater flexibility compared to fixed mean link functions.
...
Predictor selection was guided by half-normal residual plots with simulated envelopes, pseudo-R², and information criteria (AIC, SIC, HQIC).
...
The RESET (Regression Specification Error Test) checks for functional form misspecification:

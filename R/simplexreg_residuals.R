...
#' (1996) as
#' \deqn{r_i^Q = \Phi^{-1}(F(y_i; \hat{\mu}_i, \hat{\sigma}^2_i)),}
...
#' Nelder (1989) as
#' \deqn{r_i^P = \dfrac{y_i - \hat{\mu}_i}{\sqrt{\widehat{\text{Var}}(y_i)}},}
#' where \eqn{\widehat{\text{Var}}(y_i)} is the estimated variance of the response
#' (see \code{\link{variance.simplex}}).
#'
#' \strong{Deviance residuals} (\code{"deviance"}): Defined in Jørgensen (1997,
#' p. 115) as
#' \deqn{r_i^D = \dfrac{y_i - \hat{\mu}_i}{\hat{\mu}_i(1-\hat{\mu}_i)\sqrt{y_i(1-y_i)}}.}
#'
#' \strong{Standardized residuals} (\code{"standardized"}): Proposed by Espinheira
#' and Silva (2020, Eq. 15) as
#' \deqn{r_i^\beta = \dfrac{\hat{u}_i(y_i - \hat{\mu}_i)}{\sqrt{\hat{\sigma}^2_i \hat{w}_i}},}
#' where
#' \eqn{\hat{w}_i = 3\hat{\sigma}^2_i[\hat{\mu}_i(1-\hat{\mu}_i)]^{-1} + [\hat{\mu}_i(1-\hat{\mu}_i)]^{-3}}
#' and \eqn{\hat{u}_i = \hat{d}_i[\hat{\mu}_i(1-\hat{\mu}_i)]^{-1} + [\hat{\mu}_i(1-\hat{\mu}_i)]^{-3}},
#' with \eqn{\hat{d}_i} is the estimated unit deviance (see \code{\link{dev.unit.simplex}}).
#'
#' \strong{Weighted residuals} (\code{"weighted"}): Proposed by Espinheira and
#' Silva (2020, Eq. 16) as
#' \deqn{r_i^{\beta*} = \dfrac{\hat{u}_i(y_i - \hat{\mu}_i)}{\sqrt{\hat{\sigma}^2_i \hat{w}_i(1-\hat{h}_{ii})}},}
#' where \eqn{\hat{h}_{ii}} are the diagonal elements of the hat matrix
...
#' (2021, Eq. 7) as
#' \deqn{r_i^\gamma = \dfrac{\hat{d}_i - \hat{\sigma}^2_i}{\hat{\sigma}^2_i\sqrt{2}}.}
#'
#' \strong{Bias–variance residuals} (\code{"biasvariance"}): Proposed by Espinheira
#' et al. (2021, Eq. 8) as
#' \deqn{r_i^{\beta \gamma} = \dfrac{\hat{u_i}(y_i - \hat{\mu}_i) + \hat{a}_i}{\sqrt{\hat{\sigma}^2_i
#' \hat{w}_i + 1/(2\hat{\sigma}^4_i)}},} where
#' \eqn{\hat{a}_i = -(2\hat{\sigma}^2_i)^{-1} + \hat{d}_i(2\hat{\sigma}^4_i)^{-1}}.
#'
#' \strong{Score residuals} (\code{"score"}): Defined in Jørgensen (1997, p. 115) as
#' \deqn{r_i^{S} = \dfrac{(y_i - \hat{\mu}_i)(\hat{\mu}_i^2 + y_i - 2y_i\hat{\mu}_i)}
#' {y_i(1-y_i)\hat{\mu}_i^{1.5}(1-\hat{\mu}_i)^{1.5}}.}
#'
#' \strong{Dual score residuals} (\code{"dualscore"}): Defined in Jørgensen
#' (1997, p. 115) as
#' \deqn{r_i^{DS} = \dfrac{(y_i - \hat{\mu}_i)(y_i + \hat{\mu}_i - 2y_i\hat{\mu}_i)}
#' {2\sqrt{y_i(1-y_i)}\hat{\mu}_i^2(1-\hat{\mu}_i)^2}.}
#'
#' \strong{Response residuals} (\code{"response"}): Simple difference between
#' observed and fitted values, \deqn{r_i^R = y_i - \hat{\mu}_i.}
#'
